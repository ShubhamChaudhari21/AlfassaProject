...
function BodySection() {
  ...
  useEffect(() => { //Initially load datainto dataStore
    axios
  ...
  useEffect(() => { //filter useful data from dataStore
    let dataItems=[];
    if(dataStore !== undefined)
    {
      dataStore.map((obj) => {
        dataItems.push(
          (obj.elements).filter( // filter necessary elements from object array
            (nestedObj)=>{
              return nestedObj.name==="title" || nestedObj.name==="link" || 
              nestedObj.name==="pubDate" || nestedObj.name==="content:encoded" || 
              nestedObj.name==="description"
            }))
      })
      let filterDataArray=[]
      dataItems.map(element => { // filter and extract specific data
        let dataObj=
        {
            title : (element.filter(el=>{return el.name==="title"}))[0].hasOwnProperty('elements') ? (element.filter(el=>{return el.name==="title"}))[0].elements[0].text : "",
            link : (element.filter(el=>{return el.name==="link"}))[0].hasOwnProperty('elements') ? (element.filter(el=>{return el.name==="link"}))[0].elements[0].text : "",
            pubDate : (element.filter(el=>{return el.name==="pubDate"}))[0].hasOwnProperty('elements') ? (element.filter(el=>{return el.name==="pubDate"}))[0].elements[0].text : "",
            image : (element.filter(el=>{return el.name==="content:encoded"}))[0].hasOwnProperty('elements') ? (element.filter(el=>{return el.name==="content:encoded"}))[0].elements[0].cdata : "",
            description : (element.filter(el=>{return el.name==="description"}))[0].hasOwnProperty('elements') ? (element.filter(el=>{return el.name==="description"}))[0].elements[0].cdata : ""  
        }
